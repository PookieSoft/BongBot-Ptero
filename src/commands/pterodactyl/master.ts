...
export default {
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option.setName('server_name').setDescription('A friendly name for this server').setRequired(true)
                )
                .addStringOption((option) =>
                    option.setName('server_url').setDescription('The URL of your Pterodactyl panel').setRequired(true)
                )
                // TODO: [EXTRAS 2.1] Accept API key via Modal instead of visible slash command option
                .addStringOption((option) =>
                    option.setName('api_key').setDescription('Your Pterodactyl API key').setRequired(true)
                )
        )
        .addSubcommand((subcommand) => subcommand.setName('list').setDescription('List all your registered servers'))
        .addSubcommand((subcommand) =>
            subcommand
                .setName('manage')
                .setDescription('View the status and manage your servers')
                .addStringOption((option) =>
                    option.setName('server_name').setDescription('The name of the server to manage').setRequired(false)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option.setName('server_name').setDescription('The name of the server to update').setRequired(true)
                )
                .addStringOption((option) =>
                    option
                        .setName('server_url')
                ...
                .addStringOption((option) =>
                    option.setName('api_key').setDescription('The new API key').setRequired(false)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option.setName('server_name').setDescription('The name of the server to remove').setRequired(true)
                )
        ),
    ...
    fullDesc: {
        description:
            'Manage your Pterodactyl panel servers. Use subcommands to register, list, view status, update, or remove servers. View the full guide [here](https://docs.google.com/document/d/1Zp2gsq3bqzJwQ6OeA4nu_3XM3is3-TM8ynA1vWxIZL8/edit?tab=t.0&usp=sharing).',
        options: [
            {
                name: 'register',
